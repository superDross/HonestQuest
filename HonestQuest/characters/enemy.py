...
class Enemy(Character):
    ...
    def __init__(
        self, species, hp, mp, st, ag, lv, exp, gold, random, animation, magic_names
    ):
        """
        Parameters:
...
class EnemyFactory(object):
    ...
    def generate_enemy(self, species=None, lv=None):
        ...
        enemy = Enemy(
            species=self._species_dict["species"],
    ...
    def species(self, species):
        ...
        self.magic_names = {
            k: v for k, v in self._species_dict.items() if not isinstance(v, int)
        }
    ...
    def _determine_species(self, species):
        ...
        if not species:
            species2rate = {
                k: int(v["random"]) for k, v in self._species_stats_dict().items()
            }
