...
class MagicMenu(Menu):
    ...
    def _get_all_magic(self):
        ...
        """
        all_magic = [
            x for x in dir(self.hero.magic) if not re.search(r"_|hero|character", x)
        ]
        return all_magic
    ...
    def _magic_spell_options(self):
        ...
        """
        options = {
            str(k + 1): getattr(self.hero.magic, v)
    ...
    def _magic_spell_string(self):
        ...
        """
        all_magic_num = [
            "{}. {}".format(x + 1, y.title()) for x, y in enumerate(self.all_magic)
        ]
        return "\n".join(all_magic_num)
...
class ItemMenu(Menu):
    ...
    def _create_options(self):
        if self.hero.inventory:
            options = {str(k + 1): v for k, v in enumerate(self.hero.inventory)}
            return options
        else:
            return {}
