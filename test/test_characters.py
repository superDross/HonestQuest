...
class TestEnemyFactory(unittest.TestCase):
    ...
    def test_bat_stats(self):
        ...
        stats = [bat.hp, bat.mp, bat.ag, bat.st, bat.exp, bat.gold, bat.lv, bat.random]
...
class TestHero(unittest.TestCase):
    def test_level10_stats(self):
        ...
        stats = [HERO.hp, HERO._max_hp, HERO.mp, HERO._max_mp, HERO.ag, HERO.st]
        self.assertEqual(stats, expected)
    ...
    def test_level20_magic(self):
        hero = Hero("Dummy", 20)
        expected = ["fireball", "heal", "midheal", "rage", "snort_coke", "firewhirl"]
        magic = [x for x in dir(hero.magic) if not x.startswith("_")]
        self.assertEqual(sorted(magic), sorted(expected))
    ...
    def test_save(self):
        HERO.save()
        save_file = os.path.join(HERE.replace("test", "HonestQuest"), "save_file.pkl")
